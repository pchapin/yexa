...
#define ACTIVE_DESC descriptor_list[current_level]
    ...
    int MessageWindow::open(int row, int column)
    {
        ...
        if ((return_value = SimpleWindow::open(
                 row, column, width, height, ACTIVE_DESC.interior_attribute,
                 ACTIVE_DESC.border_type, ACTIVE_DESC.border_attribute)) != false) {
            ...
            center(row, SimpleWindow::column(), SimpleWindow::width(),
                   ACTIVE_DESC.interior_attribute, current_text);
