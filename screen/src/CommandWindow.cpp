...
namespace scr {
    ...
    bool CommandWindow::process_keystroke(int &key_code)
    {
        ...
        case K_HOME:
            ...
            if (old_cursor_row == 1)
                ...
                cursor_offset = (width - prompt.length()) + (width * (old_cursor_row - 2));
            }
            break;
        ...
        case K_END:
            old_cursor_row = cursor_row();
            if (old_cursor_row == 1)
                cursor_offset = width - prompt.length() - 1;
            else {
                cursor_offset = (width - prompt.length()) + (width * (old_cursor_row - 1) - 1);
            }
    ...
    ImageBuffer *CommandWindow::get_image()
    {
        ...
        image.copy(command_text.c_str(), text_row, text_column, command_text.length(),
                   WHITE | REV_BLACK);
        return (&image);
    }
