 ...
 *     #endif
 *
 * you can enable or disable single lines of code easily depending on the state of the macro
 * DEBUG. For example:
 *
...
class CursorMark {
  ...
  public:
    CursorMark()
    {
        scr::get_cursor_position(old_row, old_column);
        scr::set_cursor_position(1, 1);
    }
 ...
 */
static bool split_string(char *buffer, const char *delimiters, char *strings[],
                         int max_number_of_strings, int *actual_number_of_strings)
{
...
static int set_command(char *command_string)
{
    ...
    while (i < number_of_commands && !found) {
        if (strcmp(commands[i], command_string) == 0)
            found = true;
        i++;
    }
    if (found)
...
static void message_window(int error_level, const char *message)
{
    ...
    scr::window_center_coordinates(static_cast<int>(strlen(message + 2)), 3, &top_row,
                                   &top_column);
    the_window.open(top_row,                               // Center window.
 ...
 */
static int in_list(const char *name,              // Name to search for.
 ...
 */
static int in_snap_list(const char *name, // Name to search for.
        ...
        if (number_of_parts == 0)
            ...
            case 1: // LEVEL command.
                if (number_of_parts < 2) {
                    ...
                    snprintf(buffer, GENERIC_BUF_SIZE + 1, "Detail level set to %d",
                             detail_level);
            ...
            case 2: // MOVE command.
                if (window_position == DBG_TOP) {
                    command_window->move(scr::number_of_rows() - 5,
                                         command_window->column() - 1);
            ...
            case 4: // SHOW command.
                if (number_of_parts < 2) {
                    ...
                    snprintf(buffer, GENERIC_BUF_SIZE + 1, "Show ON at %s",
                             show_names[number_of_shows]);
                    message_window(DBG_MESSAGE, buffer);
                    number_of_shows++;
                }
                break;
            ...
            case 5: // SNAP command.
                if (number_of_parts < 2) {
                    ...
                    snprintf(buffer, GENERIC_BUF_SIZE + 1, "Snapshot function %s unknown",
                             parts[1]);
            ...
            case 7: // TRACE command.
                if (number_of_parts < 2) {
                    ...
                    snprintf(buffer, GENERIC_BUF_SIZE + 1, "Command garbled,  what is '%s' ?",
                             parts[1]);
                    message_window(DBG_ERROR, buffer);
                }
                break;
...
namespace scr {
     ...
     */
    Tracer::Tracer(const char *trace_name,    //!< Name of this entry point.
    ...
    )
    {
        ...
        if (in_list(trace_name, show_names, number_of_shows) == -1 &&
            (trace_state == false || detail_level < trace_level))
            return;
        show_trace = true;
    ...
    DebugWindow::DebugWindow(const char *header, int full_width, int full_height, int color)
    {
        ...
        print_text(row() - 1, column() + width() - 1 - static_cast<int>(strlen(header_buffer)),
                   strlen(header_buffer), header_buffer);
    }
