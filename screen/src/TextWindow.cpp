...
namespace scr {
    ...
    class TextWindowHideManager {
      ...
      public:
        TextWindowHideManager(TextWindow *managed, bool was_hidden)
            : hidden_state(was_hidden), managed_window(managed)
        {
            managed_window->show();
        }
    ...
    void TextWindow::print(const char *format_string, ...)
    {
        ...
        if (strlen(buffer) < static_cast<size_t>(width()))
            scr::clear(actual_row, actual_column + strlen(buffer), width() - strlen(buffer), 1,
                       color());
    ...
    void TextWindow::get(int start_row, int start_column, char *buffer, int size)
    {
        ...
        gets(row() + start_row, column() + start_column,
             size <= width() - start_column ? size : width() - start_column, color(), buffer);
    }
