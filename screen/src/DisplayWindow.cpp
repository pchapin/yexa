...
namespace scr {
     ...
     */
    void DisplayWindow::set(const char *input_title, std::list<std::string> *input_text,
     ...
     * explanatory.
     */
    bool DisplayWindow::open(int row, int column, int width, int height, int color,
                             int status_color, BoxType border, int border_color)
    {
        struct BorderPlug {
            BoxType border_type;
        ...
        static const BorderPlug plugs[] = {{DOUBLE_LINE, "\xB5 ", " \xC6", 2},
        ...
        if (SimpleWindow::open(row, column, width, height, color, border, border_color)) {
            ...
            for (active_plug = plugs;
                 ...
                 active_plug++) /* null */
                ;
            if (active_plug - plugs >= number_of_plugs)
                active_plug = &generic;

            // Display the title on the top border.
            left_column = SimpleWindow::column() +
                          (SimpleWindow::width() - static_cast<int>(strlen(title))) / 2 -
                          active_plug->size;

            right_column = left_column + static_cast<int>(strlen(title)) + active_plug->size;

            print_text(SimpleWindow::row() - 1, left_column, active_plug->size,
                       active_plug->left_plug);

            print_text(SimpleWindow::row() - 1, right_column, active_plug->size,
                       active_plug->right_plug);

            print_text(SimpleWindow::row() - 1, left_column + active_plug->size, strlen(title),
                       (char *)title);
