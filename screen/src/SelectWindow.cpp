...
namespace scr {
    ...
    bool SelectWindow::open(int row, int column, int width, int height, int color,
                            int Status_Color, BoxType border, int border_color)
    {
        highlight_color = Status_Color;
        show_bar = false;
        return DisplayWindow::open(row, column, width, height, color, Status_Color, border,
                                   border_color);
    }
    ...
    int SelectWindow::select(long forced)
    {
        ...
        do {
            ...
            case K_DOWN:
                ...
                if (current_index >= top_line + height())
                    top_line++;
                break;
