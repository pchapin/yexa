...
namespace scr {
    ...
    {
        static KeyPair key_associations[] = {KeyPair(KEY_F(1), K_F1),
        ...
        curses_key_map.insert(key_associations,
                              key_associations + sizeof(key_associations) / sizeof(KeyPair));
    }
    ...
    static int control_translation[] = {
        '*',     K_CTRLA, K_LEFT,  K_CTRLC,  K_CHOME, K_CTRLE, K_RIGHT, K_CEND, K_CTRLH,
        K_CTRLI, K_CTRLJ, K_CLEFT, K_CRIGHT, K_CTRLM, K_DOWN,  K_CPGDN, K_UP,   K_CTRLQ,
        K_END,   K_CTRLS, K_CTRLT, K_CPGUP,  K_PGDN,  K_HOME,  K_DEL,   K_INS,  K_PGUP};

    // Mappings from normal letters to the K_ALT... codes.
    static int alt_translation[] = {K_ALTA, K_ALTB, K_ALTC, K_ALTD, K_ALTE, K_ALTF, K_ALTG,
                                    K_ALTH, K_ALTI, K_ALTJ, K_ALTK, K_ALTL, K_ALTM, K_ALTN,
                                    K_ALTO, K_ALTP, K_ALTQ, K_ALTR, K_ALTS, K_ALTT, K_ALTU,
                                    K_ALTV, K_ALTW, K_ALTX, K_ALTY, K_ALTZ};
    ...
    static int function_translation[] = {K_F10, K_F1, K_F2, K_F3, K_F4,
                                         K_F5,  K_F6, K_F7, K_F8, K_F9};

    static int shift_function_translation[] = {K_SF10, K_SF1, K_SF2, K_SF3, K_SF4,
                                               K_SF5,  K_SF6, K_SF7, K_SF8, K_SF9};

    static int control_function_translation[] = {K_CF10, K_CF1, K_CF2, K_CF3, K_CF4,
                                                 K_CF5,  K_CF6, K_CF7, K_CF8, K_CF9};

    static int alt_function_translation[] = {K_AF10, K_AF1, K_AF2, K_AF3, K_AF4,
                                             K_AF5,  K_AF6, K_AF7, K_AF8, K_AF9};
    ...
    static KeyEscape special_key_escapes[] = {
        ...
        {"[H", K_HOME},    {"[F", K_END},      {"[2~", K_INS},    {"[3~", K_DEL},
        {"[5~", K_PGUP},   {"[6~", K_PGDN},
        ...
        {"O2P", K_SF1},    {"O2Q", K_SF2},     {"O2R", K_SF3},    {"O2S", K_SF4},
    ...
    int key_wait()
    {
        ...
        if (ch == K_CTRLF) {
            ch = getch();
            if (std::isdigit(ch))
                return function_translation[ch - '0'];
            return '*';
        }
        ...
        if (ch == K_CTRLS) {
            ch = getch();
            if (std::isdigit(ch))
                return shift_function_translation[ch - '0'];
            return '*';
        }
        ...
        if (ch == K_CTRLC) {
            ch = getch();
            if (std::isdigit(ch))
                return control_function_translation[ch - '0'];
            return '*';
        }
        ...
        if (ch == K_CTRLA) {
            ch = getch();
            if (std::isdigit(ch))
                return alt_function_translation[ch - '0'];
            if (std::isalpha(ch))
                return alt_translation[std::tolower(ch) - 'a'];
            return '*';
        }
    ...
    // See comments in key_wait below. This table is necessary because the values associated
    // with the ALT+letter keys are not in any order.
    //
    static int alt_letter_table[] = {K_ALTA, K_ALTB, K_ALTC, K_ALTD, K_ALTE, K_ALTF, K_ALTG,
                                     K_ALTH, K_ALTI, K_ALTJ, K_ALTK, K_ALTL, K_ALTM, K_ALTN,
                                     K_ALTO, K_ALTP, K_ALTQ, K_ALTR, K_ALTS, K_ALTT, K_ALTU,
                                     K_ALTV, K_ALTW, K_ALTX, K_ALTY, K_ALTZ};
    static int alt_number_table[] = {K_ALT0, K_ALT1, K_ALT2, K_ALT3, K_ALT4,
    ...
    int key_wait()
    {
        ...
        if (((ch = _getch()) != 0) && (ch != 0xE0)) {
            ...
            if (key_state & 0x8000) {
                if ('a' <= ch && ch <= 'z')
                    ch = alt_letter_table[ch - 'a'];
                else if ('0' <= ch && ch <= '9')
                    ch = alt_number_table[ch - '0'];
            }
            return ch;
        }
        return _getch() + XF;
