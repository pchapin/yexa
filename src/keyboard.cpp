...
int NeverEndingSource::get_keystroke()
{
    ...
    if (return_value == scr::K_CTRLQ)
        return_value = scr::key() | 0x8000;
    return return_value;
}
...
class RepeatSequence : public KeyboardScript {
  ...
  public:
    RepeatSequence(KeyboardScript *source) : get_count(true), key_source(source) {}
