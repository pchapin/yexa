...
namespace FileList {
    ...
    bool new_file(const char *name)
    {
        ...
        while ((end_pointer > name) && (*end_pointer != '.') && (*end_pointer != '\\'))
            end_pointer--;
        if (*end_pointer == '.')
            std::strcpy(raw_extension, end_pointer);
        else
            raw_extension[0] = '\0';
        ...
        for (i = 0; default_attributes[i].extension[0] != '\0'; i++) {
            if (my_stricmp(raw_extension, default_attributes[i].extension) == 0)
                break;
        }
        ...
        switch (this_type) {
        case ADA:
            new_thing = new ADA_YEditFile(name);
            break;
        case ASM:
            new_thing = new ASM_YEditFile(name);
            break;
        case C:
            new_thing = new C_YEditFile(name);
            break;
        case DOC:
            new_thing = new DOC_YEditFile(name);
            break;
        case PCD:
            new_thing = new PCD_YEditFile(name);
            break;
        case SCALA:
            new_thing = new SCALA_YEditFile(name);
            break;
        case OTHER:
    ...
    bool insert_active(const char *new_name)
    {
        ...
        if (return_value == true) {
            ...
            for (long line_number = top; return_value == true && line_number <= bottom;
                 line_number++) {
