...
void read_yfile()
{
    ...
    std::FILE *yfile;
    if ((yfile = std::fopen("filelist.yfy", "r")) == nullptr)
        return;
    ...
    while (std::fgets(line, linebuf_size + 2, yfile) != nullptr) {
        ...
        case 0: // ACTIVE
            if (new_descriptor != nullptr) {
                if (my_stricmp(p, "true") == 0)
                    new_descriptor->active_flag = true;
                else if (my_stricmp(p, "false") == 0)
                    new_descriptor->active_flag = false;
            }
            break;
        ...
        case 1: // BLOCK
            if (new_descriptor != nullptr) {
                if (my_stricmp(p, "true") == 0)
                    new_descriptor->block_flag = true;
                else if (my_stricmp(p, "false") == 0)
                    new_descriptor->block_flag = false;
            }
            break;
        ...
        case 6: // DELETED
            if (new_descriptor != nullptr) {
                if (my_stricmp(p, "true") == 0)
                    new_descriptor->deleted_flag = true;
                else if (my_stricmp(p, "false") == 0)
        ...
        case 7: // INSERT
            if (new_descriptor != nullptr) {
                if (my_stricmp(p, "true") == 0)
                    new_descriptor->insert_flag = true;
                else if (my_stricmp(p, "false") == 0)
                    new_descriptor->insert_flag = false;
            }
            break;
